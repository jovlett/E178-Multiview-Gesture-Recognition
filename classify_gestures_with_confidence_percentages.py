...
def get_ensemble_confidence_stats(models, X_all, ensemble_labels):
    """
    For each frame, get the confidence of the agreed-upon class label.
    Returns:
        avg_conf   : mean confidence across models for the winning class
        min_conf   : lowest confidence across models for the winning class
    """
    all_class_confs = []
    for model in models.values():
        probs = model.predict_proba(X_all)          # (N, n_classes)
        all_class_confs.append(probs)               # list of (N, n_classes)

    n = len(ensemble_labels)
    avg_conf_arr = np.zeros(n)
    min_conf_arr = np.zeros(n)

    classes = list(list(models.values())[0].classes_)

    for i, label in enumerate(ensemble_labels):
        if label == UNKNOWN:
            avg_conf_arr[i] = np.nan
            min_conf_arr[i] = np.nan
        else:
            cls_idx = classes.index(label)
            per_model_conf = [m[i, cls_idx] for m in all_class_confs]
            avg_conf_arr[i] = np.mean(per_model_conf)
            min_conf_arr[i] = np.min(per_model_conf)

    return avg_conf_arr, min_conf_arr
...
def plot_per_pose_confidence(results_df):
    ...
    model_cols = {
        "Logistic Reg.": "logistic_regression_confidence_pct",
        "MLP":           "mlp_confidence_pct",
        "Random Forest": "random_forest_confidence_pct",
    }
    model_colors = {
        "Logistic Reg.": "#4c72b0",
        "MLP":           "#dd8452",
    ...
    for i, pose in enumerate(poses):
        ...
        for model_name, col in model_cols.items():
            ...
            if len(vals) > 0:
                ...
                ax.plot(
                    [x_base - 0.25, x_base + 0.25],
                    [median, median],
                    color=model_colors[model_name],
                    linewidth=2.5, zorder=4
                )

        ax.set_title(f"{pose}\n(n={len(pose_df)})", fontsize=10)
        ax.set_xticks(list(x_positions.values()))
        ax.set_xticklabels(list(x_positions.keys()), rotation=25, ha="right", fontsize=8)
        ax.set_ylim(0, 105)
...
def main():
    ...
    # Per-model confidence for the classified label (UNKNOWN rows get NaN)
    classes = list(list(models.values())[0].classes_)
    model_col_keys = ["logistic_regression", "mlp", "random_forest"]

    for (name, model), col_key in zip(models.items(), model_col_keys):
        probs = model.predict_proba(X_all)          # (N, n_classes)
        label_conf = np.full(len(results), np.nan)

        for i, label in enumerate(results["ensemble_open_set"]):
            if label != UNKNOWN:
                cls_idx = classes.index(label)
                label_conf[i] = round(probs[i, cls_idx] * 100, 2)

        results[f"{col_key}_confidence_pct"] = label_conf
    ...
    plot_confidence_distribution(results, confidences)
    plot_agreement_scatter(results, confidences)
    plot_per_pose_confidence(results)   # ← add this
