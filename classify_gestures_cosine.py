...
os.makedirs("figures/with cosine model",  exist_ok=True)
os.makedirs("outputs/with cosine model",  exist_ok=True)
...
# ── Plots ──────────────────────────────────────────────────────────────────────
def plot_cv_comparison(cv_summary, save_path="figures/with cosine model/cv_comparison.png"):
...
def plot_confusion_matrices(models, proto, X, y,
                            save_path="figures/with cosine model/confusion_matrices.png"):
...
def plot_open_set_results(results_df, save_path="figures/with cosine model/open_set_results.png"):
    """
    Two panels:
...
def plot_confidence_vs_agreement(results_df, confidences,
                                 save_path="figures/with cosine model/confidence_vs_agreement.png"):
    """
...
def main():
    ...
    cv_summary.to_csv("outputs/with cosine model/cv_summary.csv")
    ...
    # 6. Save + plots
    results.to_csv("outputs/with cosine model/predictions_open_set.csv", index=False)
    print("\n  Saved → outputs/with cosine model/predictions_open_set.csv")
